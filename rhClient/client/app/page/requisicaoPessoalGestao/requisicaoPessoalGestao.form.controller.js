(function () {
    ...
    angular.module('app.page')
        .controller('requisicaoPessoalGestaoFormCtrl', ['$scope', '$http', 'configValue', '$timeout', '$mdDialog', '$q', 'FileUploader', '$mdToast', '$location', '$state', '$rootScope', 'GenericoService', 'AutenticacaoService', 'EnumService', requisicaoPessoalGestaoFormCtrl]);

    function requisicaoPessoalGestaoFormCtrl($scope, $http, configValue, $timeout, $mdDialog, $q, FileUploader, $mdToast, $location, $state, $rootScope, GenericoService, AutenticacaoService, EnumService) {
        ...
        if ($state.params && $state.params.id) {
            ...
            GenericoService.GetById('/requisicaoPessoalGestao/' + $state.params.id, function (response) {
                ...
                if (response.status === 200) {
                    ...
                    if($scope.funcionarioSolicitante.lotacaoId) {
                    	$scope.preencheLotacao();
                    }else {
                    	$scope.lotacaoResponsavel = 'Nenhuma lotação cadastrada';
                    }
        ...
        $scope.preencheLotacao = function () {
        	GenericoService.GetById('/lotacao/' + $scope.funcionarioSolicitante.lotacaoId, function (response) {
        		if(response.status === 200){
        			$scope.lotacaoResponsavel = response.data.descricao;
        		}
        	});
        }
        ...
        $scope.save = function (situacao) {
            ...
            if ($scope.requisicaoPessoal.id) {
                GenericoService.Update('/requisicaoPessoalGestao', $scope.requisicaoPessoal, function (response) {
                    $rootScope.$broadcast('preloader:hide');
                    if (response.status === 201 && response.data.success) {
                        ...
                        $scope.showSimpleToast(response.data.message);
                        $location.path('requisicaoPessoalGestao/formulario/' + $state.params.id)
                    }
                });
            } else {
                GenericoService.Create('/requisicaoPessoalGestao', $scope.requisicaoPessoal, function (response) {
                    $rootScope.$broadcast('preloader:hide');
                    if (response.status === 201 && response.data.success) {
                        $scope.showSimpleToast(response.data.message);
                        $location.path('/requisicaoPessoalGestao/gestao');
                        ...
                        $scope.showSimpleToast(response.data.message);
                        $location.path('/requisicaoPessoalGestao/formulario');
                    }
                });
            }
        }
