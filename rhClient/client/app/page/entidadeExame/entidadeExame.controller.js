(function () {
    ...
    angular.module('app.page')
        .controller('entidadeExameCtrl', ['configValue', '$scope', '$mdToast', '$mdDialog', '$rootScope', 'GenericoService', entidadeExameCtrl]);

    function entidadeExameCtrl(configValue, $scope, $mdToast, $mdDialog, $rootScope, GenericoService) {
        ...
        $scope.loadList = function () {
            ...
            var config = {
                params: {
                    ...
                    razaoSocial: $scope.razaoSocialBusca
                }
            };

            $scope.promise = GenericoService.GetAll('/entidadeExames', config).then(
                function (response) {
                    if (response.status === 200) {
        ...
        $scope.deleteItem = function () {
            $rootScope.$broadcast('preloader:active');
            GenericoService.Delete('/entidadeExame/' + $scope.idToDelete, function (response) {
                $rootScope.$broadcast('preloader:hide');
                if (response.status === 200) {
        ...
        $scope.showRelatorio = function () {
            $rootScope.$broadcast('preloader:active');
            GenericoService.GetAll('/listaEntidadeExames').then(
                function (response) {
                    if (response.status === 200) {
        ...
        function getDocDefinition(content, dataURL) {
            ...
            var titulo = { text: 'Tabela de Entidades de Exames', alignment: 'center', margin: [0, 10] };
                ...
                table: {
                    ...
                    body: [
                        [
                            ...
                            { text: 'CNPJ', fontSize: 10, fillColor: 'lightgray', style: 'tableHeader', alignment: 'center' },
                            { text: 'Tipo', fontSize: 10, fillColor: 'lightgray', style: 'tableHeader', alignment: 'center' }
                        ]
                    ]
                },
                layout: {
            ...
            for (i = 0; i < content.length; i++) {
                ...
                var body = [
                    { text: element.tomadorServico.razaoSocial, fontSize: 10, alignment: 'center' },
                    { text: element.tomadorServico.cnpj, fontSize: 10 },
                    { text: element.tipo, fontSize: 10 }
                ]
