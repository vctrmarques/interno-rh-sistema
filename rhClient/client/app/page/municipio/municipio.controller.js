(function () {
    ...
    angular.module('app.page')
        .controller('municipioCtrl', ['configValue', '$scope', '$mdToast', '$mdDialog', '$rootScope', '$location', 'GenericoService', municipioCtrl]);

    function municipioCtrl(configValue, $scope, $mdToast, $mdDialog, $rootScope, $location, GenericoService) {
        ...
        $scope.loadList = function () {
            ...
            $scope.promise = GenericoService.GetAll('/municipios', config).then(
                function (response) {
                    if (response.status === 200) {
        ...
        $scope.deleteItem = function () {
            $rootScope.$broadcast('preloader:active');
            GenericoService.Delete('/municipio/'+ $scope.idToDelete, function (response) {
                $rootScope.$broadcast('preloader:hide');
                if (response.status === 200) {
        ...
        $scope.showRelatorio = function () {
            $rootScope.$broadcast('preloader:active');
            GenericoService.GetAllRelatorio('/municipios').then(
                function (response) {
                    if (response.status === 200) {
        ...
        function getDocDefinition(content, dataURL) {
            ...
            var titulo = { text: 'Tabela de Municípios', alignment: 'center', margin: [0, 10] };
            ...
            var lista = {
                ...
                table: {
                    ...
                    widths: ['auto', '*', '*', '*'],
                    body: [
                        [{ text: 'UF', fontSize: 10, fillColor: 'lightgray', style: 'tableHeader', alignment: 'center' },
                        { text: 'Região Fiscal', fontSize: 10, fillColor: 'lightgray', style: 'tableHeader', alignment: 'center' },
                        { text: 'CEP', fontSize: 10, fillColor: 'lightgray', style: 'tableHeader', alignment: 'center' },
            ...
            for (i = 0; i < content.length; i++) {
                ...
                var body = [
                    { text: element.uf, fontSize: 10, alignment: 'center' },
                    { text: element.regiaoFiscal, fontSize: 10 },
                    { text: element.cep, fontSize: 10 },
