(function () {
    ...
    angular.module('app.page')
        .controller('referenciaSalarialCtrl', ['configValue', '$scope', '$mdToast', '$mdDialog', '$rootScope', 'GenericoService', '$filter', referenciaSalarialCtrl]);

    function referenciaSalarialCtrl(configValue, $scope, $mdToast, $mdDialog, $rootScope, GenericoService, $filter) {
        ...
        $scope.loadList = function () {
            ...
            $scope.promise = GenericoService.GetAll('/niveisSalariais', config).then(
                function (response) {
                    if (response.status === 200) {
        ...
        $scope.deleteItem = function () {
            $rootScope.$broadcast('preloader:active');
            GenericoService.Delete('/nivelSalarial/' + $scope.idToDelete, function (response) {
                $rootScope.$broadcast('preloader:hide');
                if (response.status === 200) {
        ...
        $scope.showRelatorio = function () {
            $rootScope.$broadcast('preloader:active');
            GenericoService.GetAllRelatorio('/niveisSalariais').then(
                function (response) {
                    if (response.status === 200) {
        ...
        function getDocDefinition(content, dataURL) {
            ...
            var titulo = {
                text: 'Tabela de Referências Salariais',
                alignment: 'center',
                margin: [0, 10]
            };
