(function () {
    ...
    angular.module('app.page')
        .controller('equipamentoProtecaoColetivaCtrl', ['configValue', '$scope', '$mdToast', '$mdDialog', '$rootScope', '$location', 'GenericoService', equipamentoProtecaoColetivaCtrl]);

    function equipamentoProtecaoColetivaCtrl(configValue, $scope, $mdToast, $mdDialog, $rootScope, $location, GenericoService) {
        ...
        $scope.loadList = function () {
            ...
            $scope.promise = GenericoService.GetAll('/equipamentoProtecaoColetiva', config).then(
                function (response) {
                    if (response.status === 200) {
        ...
        $scope.deleteItem = function () {
            $rootScope.$broadcast('preloader:active');
            GenericoService.Delete('/equipamentoProtecaoColetiva/'+ $scope.idToDelete, function (response) {
                $rootScope.$broadcast('preloader:hide');
                if (response.status === 200) {
        ...
        $scope.showRelatorio = function () {
            $rootScope.$broadcast('preloader:active');
            GenericoService.GetAllRelatorio('/equipamentoProtecaoColetiva').then(
                function (response) {
                    if (response.status === 200) {
        ...
        function getDocDefinition(content, dataURL) {
            ...
            var titulo = { text: 'Tabela de Equipamentos de Proteção Coletiva', alignment: 'center', margin: [0, 10] };
            ...
            var lista = {
                ...
                table: {
                    ...
                    body: [
                        ...
                        { text: 'Tipo de Proteção', fontSize: 10, fillColor: 'lightgray', style: 'tableHeader', alignment: 'center' },
                        { text: 'Descrição', fontSize: 10, fillColor: 'lightgray', style: 'tableHeader', alignment: 'center' },
                        { text: 'Validade', fontSize: 10, fillColor: 'lightgray', style: 'tableHeader', alignment: 'center'  },
                        { text: 'Certificado', fontSize: 10, fillColor: 'lightgray', style: 'tableHeader', alignment: 'center' },
                        { text: 'Referência', fontSize: 10, fillColor: 'lightgray', style: 'tableHeader', alignment: 'center' }
                        ]
                    ]
                },
                layout: {
            ...
            for (i = 0; i < content.length; i++) {
                ...
                var body = [
                    ...
                    { text: element.tipoProtecao, fontSize: 10 },
                    { text: element.descricao, fontSize: 10 },
                    { text: element.validade, fontSize: 10 },
                    { text: element.certificado, fontSize: 10 },
                    { text: element.referencia, fontSize: 10 }
                ]
