(function () {
    ...
    angular.module('app.page')
        .controller('areaFormacaoCtrl', ['configValue', '$scope', '$mdToast', '$mdDialog', '$rootScope', 'GenericoService', areaFormacaoCtrl]);

    function areaFormacaoCtrl(configValue, $scope, $mdToast, $mdDialog, $rootScope, GenericoService) {
        ...
        $scope.limpaFiltro = function () {
            delete $scope.areaFormacaoBusca;
            $scope.loadList();
        }
        ...
        $scope.loadList = function () {
            ...
            var config = {
                params: {
                    ...
                    areaFormacao: $scope.areaFormacaoBusca
                }
            };

            $scope.promise = GenericoService.GetAll('/areasFormacao', config).then(
                function (response) {
                    if (response.status === 200) {
        ...
        $scope.deleteItem = function () {
            $rootScope.$broadcast('preloader:active');
            GenericoService.Delete('/areaFormacao/' + $scope.idToDelete, function (response) {
                $rootScope.$broadcast('preloader:hide');
                if (response.status === 200) {
        ...
        $scope.showRelatorio = function () {
            $rootScope.$broadcast('preloader:active');
            GenericoService.GetAllRelatorio('/areasFormacao').then(
                function (response) {
                    if (response.status === 200) {
        ...
        function getDocDefinition(content, dataURL) {
            ...
            var titulo = { text: 'Tabela de Áreas de Formação', alignment: 'center', margin: [0, 10] };
            ...
            var lista = {
                ...
                table: {
                    ...
                    widths: ['*'],
                    body: [
                        [{ text: 'AreaFormacao', fontSize: 10, fillColor: 'lightgray', style: 'tableHeader', alignment: 'center'}
                        ]
                    ]
                },
                layout: {
            ...
            for (i = 0; i < content.length; i++) {
                ...
                var body = [
                    { text: element.areaFormacao, fontSize: 10 }
                ]
