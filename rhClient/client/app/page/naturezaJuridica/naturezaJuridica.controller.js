(function () {
    ...
    angular.module('app.page')
        .controller('naturezaJuridicaCtrl', ['configValue', '$scope', '$mdToast', '$mdDialog', '$rootScope', 'GenericoService', naturezaJuridicaCtrl]);

    function naturezaJuridicaCtrl(configValue, $scope, $mdToast, $mdDialog, $rootScope, GenericoService) {
        ...
        $scope.loadList = function () {
            ...
            var config = {
                params: {
                    ...
                    codigo: $scope.codigoBusca
                }
            };

            $scope.promise = GenericoService.GetAll('/naturezasJuridicas', config).then(
                function (response) {
                    if (response.status === 200) {
        ...
        $scope.deleteItem = function () {
            $rootScope.$broadcast('preloader:active');
            GenericoService.Delete('/naturezaJuridica/' + $scope.idToDelete, function (response) {
                $rootScope.$broadcast('preloader:hide');
                if (response.status === 200) {
        ...
        $scope.showRelatorio = function () {
            $rootScope.$broadcast('preloader:active');
            GenericoService.GetAllRelatorio('/naturezasJuridicas').then(
                function (response) {
                    if (response.status === 200) {
        ...
        function getDocDefinition(content, dataURL) {
            ...
            var titulo = { text: 'Relatório de Natureza Jurídica', alignment: 'center', margin: [0, 10] };
            ...
            var lista = {
                ...
                table: {
                    ...
                    body: [
                        ...
                        { text: 'Grupamento', fontSize: 10, fillColor: 'lightgray', style: 'tableHeader', alignment: 'center' }
                        ]
                    ]
                },
                layout: {
            ...
            for (i = 0; i < content.length; i++) {
                ...
                var body = [
                    ...
                    { text: element.grupamento, fontSize: 10 }
                ]
