(function () {
    ...
    angular.module('app.page')
        .controller('tomadorServicoCtrl', ['configValue', '$scope', '$mdToast', '$mdDialog', '$rootScope', 'GenericoService', tomadorServicoCtrl]);

    function tomadorServicoCtrl(configValue, $scope, $mdToast, $mdDialog, $rootScope, GenericoService) {
        ...
        $scope.loadList = function () {
            ...
            var config = {
                params: {
                    ...
                    cnpj: $scope.cnpjBusca
                }
            };

            $scope.promise = GenericoService.GetAll('/tomadores', config).then(
                function (response) {
                    if (response.status === 200) {
        ...
        $scope.deleteItem = function () {
            $rootScope.$broadcast('preloader:active');
            GenericoService.Delete('/tomadorServico/' + $scope.idToDelete, function (response) {
                $rootScope.$broadcast('preloader:hide');
                if (response.status === 200) {
        ...
        $scope.showRelatorio = function () {
            $rootScope.$broadcast('preloader:active');
            GenericoService.GetAllRelatorio('/tomadores').then(
                function (response) {
                    if (response.status === 200) {
        ...
        function getDocDefinition(content, dataURL) {
            ...
            var titulo = { text: 'Relatório de Tomadores de Serviço', alignment: 'center', margin: [0, 10] };
            ...
            var lista = {
                ...
                table: {
                    ...
                    widths: ['*', '*'],
                    body: [
                        [{ text: 'Razão Social', fontSize: 10, fillColor: 'lightgray', style: 'tableHeader', alignment: 'center' },
            ...
            for (i = 0; i < content.length; i++) {
                ...
                var body = [
                    { text: element.razaoSocial, fontSize: 10, alignment: 'center' },
                    { text: element.cnpj, fontSize: 10 }
                ]
