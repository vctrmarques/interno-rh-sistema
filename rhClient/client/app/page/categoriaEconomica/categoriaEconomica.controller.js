(function () {
    ...
    angular.module('app.page')
        .controller('categoriaEconomicaCtrl', ['configValue', '$scope', '$mdToast', '$mdDialog', '$rootScope', '$location', 'GenericoService', categoriaEconomicaCtrl]);

    function categoriaEconomicaCtrl(configValue, $scope, $mdToast, $mdDialog, $rootScope, $location, GenericoService) {
        ...
        $scope.loadList = function () {
            ...
            $scope.promise = GenericoService.GetAll('/categoriasEconomicas', config).then(
                function (response) {
                    if (response.status === 200) {
        ...
        $scope.deleteItem = function () {
            $rootScope.$broadcast('preloader:active');
            GenericoService.Delete('/categoriaEconomica/'+ $scope.idToDelete, function (response) {
                $rootScope.$broadcast('preloader:hide');
                if (response.status === 200) {
        ...
        $scope.showRelatorio = function () {
            $rootScope.$broadcast('preloader:active');
            GenericoService.GetAllRelatorio('/categoriasEconomicas').then(
                function (response) {
                    if (response.status === 200) {
        ...
        function getDocDefinition(content, dataURL) {
            ...
            var titulo = { text: 'Tabela de Categorias Econômicas', alignment: 'center', margin: [0, 10] };
