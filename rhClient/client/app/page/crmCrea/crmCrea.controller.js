(function () {
    ...
    angular.module('app.page')
        .controller('crmCreaCtrl', ['configValue', '$scope', '$mdToast', '$mdDialog', '$rootScope', 'GenericoService', crmCreaCtrl]);

    function crmCreaCtrl(configValue, $scope, $mdToast, $mdDialog, $rootScope, GenericoService) {
        ...
        $scope.loadList = function () {
            ...
            var config = {
                params: {
                    ...
                    nomeConveniado: $scope.nomeConveniadoBusca
                }
            };

            $scope.promise = GenericoService.GetAll('/crmCreas', config).then(
                function (response) {
                    if (response.status === 200) {
        ...
        $scope.deleteItem = function () {
            $rootScope.$broadcast('preloader:active');
            GenericoService.Delete('/crmCrea/' + $scope.idToDelete, function (response) {
                $rootScope.$broadcast('preloader:hide');
                if (response.status === 200) {
        ...
        $scope.showRelatorio = function () {
            ...
            GenericoService.GetAll('/crmCreas', config).then(
                function (response) {
                    if (response.status === 200) {
        ...
        function getDocDefinition(content, dataURL) {
            ...
            var titulo = { text: 'Tabela de CRM / CREA', alignment: 'center', margin: [0, 10] };
            ...
            var lista = {
                ...
                table: {
                    ...
                    body: [
                        [
                            { text: 'Número CRM / CREA', fontSize: 10, fillColor: 'lightgray', style: 'tableHeader', alignment: 'center' },
                            { text: 'Nome conveniado', fontSize: 10, fillColor: 'lightgray', style: 'tableHeader', alignment: 'center' }
            ...
            for (i = 0; i < content.length; i++) {
                ...
                var body = [
                    { text: element.numeroCrmCrea, fontSize: 10, alignment: 'center' },
                    { text: element.nomeConveniado, fontSize: 10 }
                ]
