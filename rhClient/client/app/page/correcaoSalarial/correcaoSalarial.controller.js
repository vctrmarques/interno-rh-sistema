(function () {
    ...
    angular.module('app.page')
        .controller('correcaoSalarialCtrl', ['configValue', '$scope', '$mdToast', '$mdDialog', '$rootScope', 'GenericoService', correcaoSalarialCtrl]);

    function correcaoSalarialCtrl(configValue, $scope, $mdToast, $mdDialog, $rootScope, GenericoService) {
        ...
        $scope.loadList = function () {
            ...
            var config = {
                params: {
                    ...
                    nomeFilial: $scope.nomeFilialBusca
                }
            };

            $scope.promise = GenericoService.GetAll('/correcoes', config).then(
                function (response) {
                    if (response.status === 200) {
        ...
        $scope.deleteItem = function () {
            $rootScope.$broadcast('preloader:active');
            GenericoService.Delete('/correcao/' + $scope.idToDelete, function (response) {
                $rootScope.$broadcast('preloader:hide');
                if (response.status === 200) {
        ...
        $scope.showRelatorio = function () {
            $rootScope.$broadcast('preloader:active');
            GenericoService.GetAllRelatorio('/correcoes').then(
                function (response) {
                    if (response.status === 200) {
        ...
        function getDocDefinition(content, dataURL) {
            ...
            var titulo = { text: 'Relatório de Correções', alignment: 'center', margin: [0, 10] };
            ...
            var lista = {
                ...
                table: {
                    ...
                    body: [
                        ...
                        { text: 'Retroativo', fontSize: 10, fillColor: 'lightgray', style: 'tableHeader', alignment: 'center'}
                        ]
                    ]
                },
                layout: {
            ...
            for (i = 0; i < content.length; i++) {
                ...
                var body = [
                    ...
                    { text: element.retroativo, fontSize: 10 }
                ]
