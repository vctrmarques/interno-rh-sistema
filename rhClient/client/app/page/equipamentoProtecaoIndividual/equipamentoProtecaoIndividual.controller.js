(function () {
    ...
    angular.module('app.page')
        .controller('equipamentoProtecaoIndividualCtrl', ['configValue', '$scope', '$mdToast', '$mdDialog', '$rootScope', '$location', 'GenericoService', equipamentoProtecaoIndividualCtrl]);

    function equipamentoProtecaoIndividualCtrl(configValue, $scope, $mdToast, $mdDialog, $rootScope, $location, GenericoService) {
        ...
        $scope.buscar = function () {
            if($scope.descricaoBusca.length>1){
                $scope.loadList();
            }
        ...
        $scope.loadList = function () {
            ...
            $scope.promise = GenericoService.GetAll('/equipamentoProtecaoIndividual', config).then(
                function (response) {
                    if (response.status === 200) {
        ...
        $scope.deleteItem = function () {
            $rootScope.$broadcast('preloader:active');
            GenericoService.Delete('/equipamentoProtecaoIndividual/'+ $scope.idToDelete, function (response) {
                $rootScope.$broadcast('preloader:hide');
                if (response.status === 200) {
        ...
        $scope.showRelatorio = function () {
            $rootScope.$broadcast('preloader:active');
            GenericoService.GetAllRelatorio('/equipamentoProtecaoIndividual').then(
                function (response) {
                    if (response.status === 200) {
        ...
        function getDocDefinition(content, dataURL) {
            ...
            var titulo = { text: 'Tabela de Equipamentos de Proteção Individual', alignment: 'center', margin: [0, 10] };
