(function () {
    ...
    angular.module('app.page')
        .controller('transferenciaPrevidenciaFuncionarioCtrl', ['configValue', '$scope', '$mdToast', '$mdDialog', '$rootScope', 'GenericoService', transferenciaPrevidenciaFuncionarioCtrl]);

    function transferenciaPrevidenciaFuncionarioCtrl(configValue, $scope, $mdToast, $mdDialog, $rootScope, GenericoService) {
        ...
        $scope.loadList = function () {
            ...
            $scope.promise = GenericoService.GetAll('/transferenciasPrevidenciaFuncionarios', config).then(
                function (response) {
                    if (response.status === 200) {
        ...
        $scope.deleteItem = function () {
            $rootScope.$broadcast('preloader:active');
            GenericoService.Delete('/tiposAposentadoria/' + $scope.idToDelete, function (response) {
                $rootScope.$broadcast('preloader:hide');
                if (response.status === 200) {
        ...
        $scope.showRelatorio = function () {
            $rootScope.$broadcast('preloader:active');
            GenericoService.GetAllRelatorio('/tiposAposentadoria').then(
                function (response) {
                    if (response.status === 200) {
        ...
        function getDocDefinition(content, dataURL) {
            ...
            var titulo = { text: 'Tipos de Aposentadorias', alignment: 'center', margin: [0, 10] };
