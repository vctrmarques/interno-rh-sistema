(function () {
    ...
    angular.module('app.page')
        .controller('acidenteTrabalhoCtrl', ['configValue', '$scope', '$mdToast', '$mdDialog', '$rootScope', 'GenericoService', acidenteTrabalhoCtrl]);

    function acidenteTrabalhoCtrl(configValue, $scope, $mdToast, $mdDialog, $rootScope, GenericoService) {
        ...
        $scope.loadList = function () {
            ...
            var config = {
                params: {
                    ...
                    nomeOrMatricula: $scope.nomeOuMatriculaBusca
                }
            };

            $scope.promise = GenericoService.GetAll('/funcionarios/acidentes/trabalho', config).then(
                function (response) {
                    if (response.status === 200) {
        ...
        $scope.deleteItem = function () {
            $rootScope.$broadcast('preloader:active');
            GenericoService.Delete('/acidenteTrabalho/' + $scope.idToDelete, function (response) {
                $rootScope.$broadcast('preloader:hide');
                if (response.status === 200) {
        ...
        $scope.showRelatorio = function () {
            ...
            GenericoService.GetAll('/acidenteTrabalhos', config).then(
                function (response) {
                    if (response.status === 200) {
