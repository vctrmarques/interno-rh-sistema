(function () {
    ...
    angular.module('app.page')
        .controller('modeloDocumentoCtrl', ['configValue', '$scope', '$mdToast', '$mdDialog', '$rootScope', 'GenericoService', '$filter', modeloDocumentoCtrl]);

    function modeloDocumentoCtrl(configValue, $scope, $mdToast, $mdDialog, $rootScope, GenericoService, $filter) {
        ...
        $scope.loadList = function () {
            ...
            $scope.promise = GenericoService.GetAll('/modelosDocumentos', config).then(
                function (response) {
                    if (response.status === 200) {
        ...
        $scope.deleteItem = function () {
            $rootScope.$broadcast('preloader:active');
            GenericoService.Delete('/modeloDocumento/' + $scope.idToDelete, function (response) {
                $rootScope.$broadcast('preloader:hide');
                if (response.status === 200) {
        ...
        $scope.showRelatorio = function () {
            $rootScope.$broadcast('preloader:active');
            GenericoService.GetAllRelatorio('/modelosDocumentos').then(
                function (response) {
                    if (response.status === 200) {
        ...
        function getDocDefinition(content, dataURL) {
            ...
            var titulo = { text: 'Tabela de Modelo de Documentos', alignment: 'center', margin: [0, 10] };
            ...
            for (i = 0; i < content.length; i++) {
                ...
                var body = [
                    ...
                    { text:  $filter('date')(element.criadoEm, 'dd/MM/yyyy HH:mm:ss'), fontSize: 10 }
                ]
