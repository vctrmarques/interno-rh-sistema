(function () {
    ...
    angular.module('app.page')
        .controller('historicoContabilCtrl', ['configValue', '$scope', '$mdToast', '$mdDialog', '$rootScope', '$location', 'GenericoService', historicoContabilCtrl]);

    function historicoContabilCtrl(configValue, $scope, $mdToast, $mdDialog, $rootScope, $location, GenericoService) {
        ...
        $scope.loadList = function () {
            ...
            $scope.promise = GenericoService.GetAll('/historicosContabeis', config).then(
                function (response) {
                    if (response.status === 200) {
        ...
        $scope.deleteItem = function () {
            $rootScope.$broadcast('preloader:active');
            GenericoService.Delete('/historicoContabil/'+ $scope.idToDelete, function (response) {
                $rootScope.$broadcast('preloader:hide');
                if (response.status === 200) {
        ...
        $scope.showRelatorio = function () {
            $rootScope.$broadcast('preloader:active');
            GenericoService.GetAllRelatorio('/historicosContábeis').then(
                function (response) {
                    if (response.status === 200) {
