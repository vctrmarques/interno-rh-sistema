(function () {
    ...
    angular.module('app.page')
        .controller('usuarioCtrl', ['configValue', '$scope', '$mdToast', '$mdDialog', '$rootScope', '$window', '$location', 'UsuarioService', 'GenericoService', usuarioCtrl]);

    function usuarioCtrl(configValue, $scope, $mdToast, $mdDialog, $rootScope, $window, $location, UsuarioService, GenericoService) {
        ...
        $scope.loadList = function () {
            ...
            $scope.promise = UsuarioService.GetAll($scope.query.page, $scope.query.limit, $scope.nomeBusca, $scope.cpfBusca, $scope.loginBusca, $scope.query.order).then(
                function (response) {
                    if (response.status === 200) {
        ...
        $scope.deleteItem = function () {
            $rootScope.$broadcast('preloader:active');
            UsuarioService.Delete($scope.idToDelete, function (response) {
                $rootScope.$broadcast('preloader:hide');
                if (response.status === 200) {
        ...
        $scope.showRelatorio = function () {
            $rootScope.$broadcast('preloader:active');
            UsuarioService.GetAllRelatorio().then(
                function (response) {
                    if (response.status === 200) {
        ...
        function getDocDefinition(content, dataURL) {
            ...
            var titulo = { text: 'Tabela de Usuários', alignment: 'center', margin: [0, 10] };
            ...
            var lista = {
                ...
                table: {
                    ...
                    body: [
                        ...
                        { text: 'Login', fontSize: 10, fillColor: 'lightgray', style: 'tableHeader', alignment: 'center' },
                        { text: 'E-mail', fontSize: 10, fillColor: 'lightgray', style: 'tableHeader', alignment: 'center' },
            ...
            for (i = 0; i < content.length; i++) {
                ...
                var body = [
                    ...
                    { text: element.login, fontSize: 10 },
                    { text: element.email, fontSize: 10 },
                    { text: element.cpf, fontSize: 10, alignment: 'center' }
                ]
