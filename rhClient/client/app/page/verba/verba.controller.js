(function () {
    ...
    angular.module('app.page')
        .controller('verbaCtrl', ['configValue', '$scope', '$mdToast', '$mdDialog', '$rootScope', 'GenericoService', verbaCtrl]);

    function verbaCtrl(configValue, $scope, $mdToast, $mdDialog, $rootScope, GenericoService) {
        ...
        $scope.loadList = function () {
            ...
            var config = {
                params: {
                    ...
                    search: $scope.descricaoVerbaBusca
                }
            };

            $scope.promise = GenericoService.GetAll('/verbas', config).then(
                function (response) {
                    if (response.status === 200) {
        ...
        $scope.deleteItem = function () {
            $rootScope.$broadcast('preloader:active');
            GenericoService.Delete('/verba/' + $scope.idToDelete, function (response) {
                $rootScope.$broadcast('preloader:hide');
                if (response.status === 200) {
        ...
        $scope.showRelatorio = function () {
            ...
            GenericoService.GetAll('/verbas', config).then(
                function (response) {
                    if (response.status === 200) {
        ...
        function getDocDefinition(content, dataURL) {
            ...
            var titulo = { text: 'Tabela de Verbas', alignment: 'center', margin: [0, 10] };
            ...
            for (i = 0; i < content.length; i++) {
                ...
                var body = [
                    ...
                    { text: element.descricaoVerba, fontSize: 10 }
                ]
