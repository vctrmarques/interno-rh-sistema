(function () {
    ...
    angular.module('app.page')
        .controller('especialidadeMedicaCtrl', ['configValue', '$scope', '$mdToast', '$mdDialog', '$rootScope', 'RestService', especialidadeMedicaCtrl]);

    function especialidadeMedicaCtrl(configValue, $scope, $mdToast, $mdDialog, $rootScope, RestService) {
        ...
        $scope.loadList = function () {
            ...
            $scope.promise = RestService.Get('/especialidadeMedica', config)
        ...
        $scope.deleteItem = function () {
            $rootScope.$broadcast('preloader:active');
            RestService.Delete('/especialidadeMedica/' + $scope.idToDelete)
                .then(function successCallback(response) {
                    $rootScope.$broadcast('preloader:hide');
                    if (response.status === 200) {
                        $scope.showSimpleToast(response.data.message);
        ...
        $scope.showRelatorio = function () {
            ...
            RestService.Get('/especialidadeMedica', config)
                .then(function successCallback(response) {
                    $rootScope.$broadcast('preloader:hide');
                    if (response.status === 200) {
        ...
        function getDocDefinition(content, dataURL) {
            ...
            var titulo = { text: 'Tabela de Especialidades Médicas', alignment: 'center', margin: [0, 10] };
