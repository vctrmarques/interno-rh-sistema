(function () {
    ...
    angular.module('app.page')
        .controller('classificacaoInternacionalDoencaCtrl', ['configValue', '$scope', '$mdToast', '$mdDialog', '$rootScope', 'GenericoService', classificacaoInternacionalDoencaCtrl]);

    function classificacaoInternacionalDoencaCtrl(configValue, $scope, $mdToast, $mdDialog, $rootScope, GenericoService) {
        ...
        $scope.loadList = function () {
            ...
            $scope.promise = GenericoService.GetAll('/classificacaoInternacionalDoencas', config).then(
                function (response) {
                    if (response.status === 200) {
        ...
        $scope.deleteItem = function () {
            $rootScope.$broadcast('preloader:active');
            GenericoService.Delete('/classificacaoInternacionalDoenca/' + $scope.idToDelete, function (response) {
                $rootScope.$broadcast('preloader:hide');
                if (response.status === 200) {
        ...
        function getDocDefinition(content, dataURL) {
            ...
            var titulo = { text: 'Tabela de Classificação Internacional de Doenças', alignment: 'center', margin: [0, 10] };
